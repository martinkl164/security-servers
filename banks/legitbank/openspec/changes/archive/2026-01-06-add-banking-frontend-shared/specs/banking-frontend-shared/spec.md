...
The system SHALL include a “cat in the bank” logo mark that is text-based and requires no image assets (inline SVG is acceptable).
...
- **AND** the header includes a cat-in-bank logo mark without relying on image files
